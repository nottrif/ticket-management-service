...
public class TrainService {
   ...
   * @throws IllegalArgumentException if the seat is not available or the user
  ...
  public ReceiptDTO purchaseTicket(User user, String section, String seat) {
    ...
    if ("A".equals(section) && sectionASeats.contains(chosenSeat)) {
      ...
      throw new IllegalArgumentException("Seat " + seat + " in section " +
    ...
    return new ReceiptDTO(ticket.getFrom(), ticket.getTo(), user,
                          ticket.getPrice());
  }
   ...
   * @return a ReceiptDTO containing the ticket details, or null if no ticket is
   *     found
   */
  public ReceiptDTO getTicket(String email) {
    Ticket ticket = allocatedTickets.get(email);
    if (ticket != null) {
      return new ReceiptDTO(ticket.getFrom(), ticket.getTo(), ticket.getUser(),
                            ticket.getPrice());
    } else {
      return null;
    }
  ...
  public Map<String, String> getUsersBySection(String section) {
    return allocatedTickets.values()
        .stream()
        .filter(ticket -> section.equals(ticket.getSection()))
        .collect(Collectors.toMap(ticket
   ...
   * @throws IllegalArgumentException if the new seat is not available or no
   *     ticket is found for the user
   */
  public ReceiptDTO modifySeat(String email, String newSection,
                               String newSeat) {
    Ticket ticket = allocatedTickets.get(email);
    if (ticket == null) {
      throw new IllegalArgumentException(
          "No ticket found for user with email " + email);
    }
    ...
    if ("A".equals(newSection) && sectionASeats.contains(seat)) {
      ...
      throw new IllegalArgumentException("Seat " + newSeat + " in section " +
                                         newSection + " is not available.");
    }
    ...
    return new ReceiptDTO(newTicket.getFrom(), newTicket.getTo(),
                          newTicket.getUser(), newTicket.getPrice());
  }
}